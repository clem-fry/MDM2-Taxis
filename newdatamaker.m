...
sample = datasample(T, 75000, 'Replace', false);
...
toDelete = sample.trip_duration > 7200; sample(toDelete,:) = [];
toDelete = sample.trip_duration < 10; sample(toDelete,:) = [];

toDelete = sample.pickup_longitude > -73.7; sample(toDelete,:) = [];
toDelete = sample.pickup_longitude < -74.2; sample(toDelete,:) = [];

toDelete = sample.dropoff_longitude > -73.7; sample(toDelete,:) = [];
toDelete = sample.dropoff_longitude < -74.2; sample(toDelete,:) = [];

toDelete = sample.pickup_latitude > 40.9; sample(toDelete,:) = [];
toDelete = sample.pickup_latitude < 40.6; sample(toDelete,:) = [];

toDelete = sample.dropoff_latitude > 40.9; sample(toDelete,:) = [];
toDelete = sample.dropoff_latitude < 40.6; sample(toDelete,:) = [];

writetable(sample,'clean_train.csv')